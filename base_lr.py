...
def generate_baseClassifiers(data, var, test_year1, test_year2, add_year1, add_year2, add_year3, add_year4, base_path, data_split):
    ...
    for i in range(20):
        for j in range(5):
            ...
            ### define column name
            col_name = 'lr_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var
            col_name1 = 'lr_'+'seed_'+str(i)+'_paras_'+var
            col_name2 = 'lr_'+'paras_'+var

            ###create classifier
            clf = LogisticRegression(C=0.1, max_iter=300, class_weight=None)
