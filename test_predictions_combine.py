...
def combine_probabilities(basepath, mcc, probability_path, var): 
    knn_base_path = basepath + '/knn/knn_' + var + '/test_class'
    lr_base_path = basepath + '/lr/lr_' + var + '/test_class'
    svm_base_path = basepath + '/svm/svm_' + var + '/test_class'
    rf_base_path = basepath + '/rf/rf_' + var + '/test_class'
    xgboost_base_path = basepath + '/xgboost/xgboost_' + var + '/test_class'
    ...
    tmp = pd.read_csv(join(knn_base_path, 'test_knn_paras_'+var+'_K_7.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    data.to_csv(join(probability_path+'/test_probabilities_'+var+'.csv'))
