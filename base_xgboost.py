...
def generate_baseClassifiers(data, var, test_year1, test_year2, add_year1, add_year2, add_year3, add_year4, base_path, data_split):
    ...
    for i in range(20):
        for j in range(5):
            ...
            print('before add:', X_train.shape)
            ...
            ### define column name
            col_name = 'xgboost_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var
            col_name1 = 'xgboost_'+'seed_'+str(i)+'_paras_'+var
            col_name2 = 'xgboost_'+'paras_'+var

            ###create classifier
            clf = XGBClassifier(learning_rate=0.01, n_estimators=700, max_depth=11, subsample=0.7, scale_pos_weight=0.66)
