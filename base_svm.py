...
def generate_baseClassifiers(data, var, test_year1, test_year2, add_year1, add_year2, add_year3, add_year4, base_path, data_split):
    ...
    for i in range(20):
        for j in range(5):
            ...
            ### define column name 
            col_name = 'svm_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var
            col_name1 = 'svm_'+'seed_'+str(i)+'_paras_'+var
            col_name2 = 'svm_'+'paras_'+var

            ### create and fit model
            clf = SVC(kernel='rbf', C=1, gamma='scale', probability=True,  class_weight = 'balanced', random_state=1)
